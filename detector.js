(function (root, factory) {
  ...
  function ensureWorker() {
    ...
    worker.onmessage = event => {
      if (event.data.progress) {
        for (const request of pending.values()) request.onProgress?.(event.data);
        return;
      }
  ...
  function send(message, transfer, timeoutMs, onProgress) {
    const id = ++requestId;
  ...
  async function detect(canvas, onProgress) {
    ...
    const result = await send({ rgba: image.data.buffer, width, height }, [image.data.buffer], 120000, onProgress);
  ...
  function warmup(onProgress) {
    ...
    return send({ type: 'warmup' }, [], 120000, onProgress);
  }
